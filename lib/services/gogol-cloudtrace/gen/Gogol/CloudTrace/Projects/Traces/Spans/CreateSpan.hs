               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] Span Core.:> Core.Post '[Core.JSON] Span
...
newCloudTraceProjectsTracesSpansCreateSpan name payload
  = CloudTraceProjectsTracesSpansCreateSpan{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            name = name, payload = payload,
                                            uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest CloudTraceProjectsTracesSpansCreateSpan
         where
        type Rs CloudTraceProjectsTracesSpansCreateSpan = Span
        type Scopes CloudTraceProjectsTracesSpansCreateSpan =
             '[CloudPlatform'FullControl, Trace'Append]
        requestClient CloudTraceProjectsTracesSpansCreateSpan{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy CloudTraceProjectsTracesSpansCreateSpanResource)
                      Core.mempty
