...
--
-- /See:/ <https://developers.google.com/workspace/drive/ Google Drive API Reference> for @drive.accessproposals.list@.
module Gogol.Drive.Accessproposals.List
    (
    -- * Resource
...
newDriveAccessproposalsList fileId
  = DriveAccessproposalsList{xgafv = Core.Nothing,
                             accessToken = Core.Nothing, callback = Core.Nothing,
                             fileId = fileId, pageSize = Core.Nothing, pageToken = Core.Nothing,
                             uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DriveAccessproposalsList where
        type Rs DriveAccessproposalsList = ListAccessProposalsResponse
        ...
        requestClient DriveAccessproposalsList{..}
          = go fileId xgafv accessToken callback pageSize pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DriveAccessproposalsListResource)
                      Core.mempty
