...
--
-- Gets a file\'s metadata or content by ID. If you provide the URL parameter @alt=media@, then the response includes the file contents in the response body. Downloading content with @alt=media@ only works if the file is stored in Drive. To download Google Docs, Sheets, and Slides use </workspace/drive/api/reference/rest/v3/files/export files.export> instead. For more information, see </workspace/drive/api/guides/manage-downloads Download & export files>.
--
-- /See:/ <https://developers.google.com/workspace/drive/ Google Drive API Reference> for @drive.files.get@.
module Gogol.Drive.Files.Get
    (
    -- * Resource
...
newDriveFilesGet fileId
  = DriveFilesGet{xgafv = Core.Nothing, accessToken = Core.Nothing,
                  acknowledgeAbuse = Core.False, callback = Core.Nothing,
...
instance Core.GoogleRequest DriveFilesGet where
        ...
        type Scopes DriveFilesGet =
             '[Drive'FullControl, Drive'Appdata, Drive'File,
               Drive'Meet'Readonly, Drive'Metadata, Drive'Metadata'Readonly,
               Drive'Photos'Readonly, Drive'Readonly]
        requestClient DriveFilesGet{..}
          = go fileId xgafv accessToken (Core.Just acknowledgeAbuse) callback
              includeLabels
              ...
              driveService
          where go Core.:<|> _
                  = Core.buildClient (Core.Proxy :: Core.Proxy DriveFilesGetResource)
                      Core.mempty

instance Core.GoogleRequest (Core.MediaDownload DriveFilesGet)
         where
        type Rs (Core.MediaDownload DriveFilesGet) = Core.Stream
        type Scopes (Core.MediaDownload DriveFilesGet) =
