...
--
-- /See:/ <https://developers.google.com/workspace/drive/ Google Drive API Reference> for @drive.files.generateIds@.
module Gogol.Drive.Files.GenerateIds
    (
    -- * Resource
...
newDriveFilesGenerateIds 
    ::  DriveFilesGenerateIds
newDriveFilesGenerateIds
  = DriveFilesGenerateIds{xgafv = Core.Nothing,
                          accessToken = Core.Nothing, callback = Core.Nothing, count = 10,
                          space = "drive", type' = "files", uploadType = Core.Nothing,
...
instance Core.GoogleRequest DriveFilesGenerateIds where
        ...
        requestClient DriveFilesGenerateIds{..}
          = go xgafv accessToken callback (Core.Just count) (Core.Just space)
              (Core.Just type')
              uploadType
              uploadProtocol
