...
--
-- /See:/ <https://developers.google.com/workspace/drive/ Google Drive API Reference> for @drive.files.modifyLabels@.
module Gogol.Drive.Files.ModifyLabels
    (
    -- * Resource
...
newDriveFilesModifyLabels fileId payload
  = DriveFilesModifyLabels{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing,
                           fileId = fileId, payload = payload, uploadType = Core.Nothing,
...
instance Core.GoogleRequest DriveFilesModifyLabels where
        ...
        requestClient DriveFilesModifyLabels{..}
          = go fileId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              driveService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DriveFilesModifyLabelsResource)
                      Core.mempty
