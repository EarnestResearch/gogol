...
--
-- /See:/ <https://developers.google.com/workspace/drive/ Google Drive API Reference> for @drive.revisions.get@.
module Gogol.Drive.Revisions.Get
    (
    -- * Resource
...
newDriveRevisionsGet fileId revisionId
  = DriveRevisionsGet{xgafv = Core.Nothing,
                      accessToken = Core.Nothing, acknowledgeAbuse = Core.False,
                      callback = Core.Nothing, fileId = fileId, revisionId = revisionId,
...
instance Core.GoogleRequest (Core.MediaDownload DriveRevisionsGet)
         where
