...
--
-- /See:/ <https://developers.google.com/workspace/drive/ Google Drive API Reference> for @drive.revisions.list@.
module Gogol.Drive.Revisions.List
    (
    -- * Resource
...
newDriveRevisionsList fileId
  = DriveRevisionsList{xgafv = Core.Nothing,
                       accessToken = Core.Nothing, callback = Core.Nothing,
                       fileId = fileId, pageSize = 200, pageToken = Core.Nothing,
...
instance Core.GoogleRequest DriveRevisionsList where
        ...
        requestClient DriveRevisionsList{..}
          = go fileId xgafv accessToken callback (Core.Just pageSize)
              pageToken
              uploadType
              uploadProtocol
