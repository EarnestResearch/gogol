         ...
         Core.QueryParam "$.xgafv" Xgafv Core.:>
           Core.QueryParam "access_token" Core.Text Core.:>
             Core.QueryParam "callback" Core.Text Core.:>
               Core.QueryParam "offset" Core.Int32 Core.:>
                 Core.QueryParam "organization" Core.Text Core.:>
...
instance Core.GoogleRequest FactCheckToolsPagesList
         where
        type Rs FactCheckToolsPagesList =
             GoogleFactcheckingFactchecktoolsV1alpha1ListClaimReviewMarkupPagesResponse
        type Scopes FactCheckToolsPagesList =
             '[Userinfo'Email]
        requestClient FactCheckToolsPagesList{..}
          = go xgafv accessToken callback offset organization
              pageSize
              pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy FactCheckToolsPagesListResource)
                      Core.mempty
