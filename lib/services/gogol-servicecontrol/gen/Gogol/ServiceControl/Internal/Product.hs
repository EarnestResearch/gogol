...
newApi 
    ::  Api
newApi
  = Api{operation = Core.Nothing, protocol = Core.Nothing,
        service = Core.Nothing, version = Core.Nothing}
instance Core.FromJSON Api where
        parseJSON
          = Core.withObject "Api"
              (\ o ->
                 Api Core.<$>
                   (o Core..:? "operation") Core.<*> (o Core..:? "protocol") Core.<*>
                     (o Core..:? "service")
                     Core.<*> (o Core..:? "version"))
...
newAttributeContext 
    ::  AttributeContext
newAttributeContext
  = AttributeContext{api = Core.Nothing, destination = Core.Nothing,
                     extensions = Core.Nothing, origin = Core.Nothing,
                     request' = Core.Nothing, resource = Core.Nothing,
                     response = Core.Nothing, source = Core.Nothing}
instance Core.FromJSON AttributeContext where
        parseJSON
          = Core.withObject "AttributeContext"
              (\ o ->
                 AttributeContext Core.<$>
                   (o Core..:? "api") Core.<*> (o Core..:? "destination") Core.<*>
                     (o Core..:? "extensions")
                     Core.<*> (o Core..:? "origin")
                     Core.<*> (o Core..:? "request")
...
instance Core.FromJSON AttributeContext_ExtensionsItem where
        parseJSON
          = Core.withObject "AttributeContext_ExtensionsItem"
              (\ o ->
                 AttributeContext_ExtensionsItem Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON AttributeContext_ExtensionsItem where
        toJSON AttributeContext_ExtensionsItem{..} = Core.toJSON additional
...
newAuditLog 
    ::  AuditLog
newAuditLog
  = AuditLog{authenticationInfo = Core.Nothing,
             authorizationInfo = Core.Nothing, metadata = Core.Nothing,
             methodName = Core.Nothing, numResponseItems = Core.Nothing,
             policyViolationInfo = Core.Nothing, request' = Core.Nothing,
             requestMetadata = Core.Nothing, resourceLocation = Core.Nothing,
             resourceName = Core.Nothing, resourceOriginalState = Core.Nothing,
             response = Core.Nothing, serviceData = Core.Nothing,
              ...
              (\ o ->
                 AuditLog Core.<$>
                   (o Core..:? "authenticationInfo") Core.<*>
                     ...
                     Core.<*>
                     (o Core..:? "numResponseItems" Core.<&> Core.fmap Core.fromAsText)
...
instance Core.ToJSON AuditLog where
        toJSON AuditLog{..}
          = Core.object
              (Core.catMaybes
                 [("authenticationInfo" Core..=) Core.<$> authenticationInfo,
                  ("authorizationInfo" Core..=) Core.<$> authorizationInfo,
                  ("metadata" Core..=) Core.<$> metadata,
                  ("methodName" Core..=) Core.<$> methodName,
                  ("numResponseItems" Core..=) Core.. Core.AsText Core.<$>
                    numResponseItems,
                  ("policyViolationInfo" Core..=) Core.<$> policyViolationInfo,
                  ("request" Core..=) Core.<$> request',
                  ("requestMetadata" Core..=) Core.<$> requestMetadata,
                  ("resourceLocation" Core..=) Core.<$> resourceLocation,
                  ("resourceName" Core..=) Core.<$> resourceName,
                  ("resourceOriginalState" Core..=) Core.<$> resourceOriginalState,
                  ("response" Core..=) Core.<$> response,
...
newAuditLog_Metadata 
    ...
    -> AuditLog_Metadata
newAuditLog_Metadata additional
  = AuditLog_Metadata{additional = additional}
instance Core.FromJSON AuditLog_Metadata where
        parseJSON
          = Core.withObject "AuditLog_Metadata"
              (\ o -> AuditLog_Metadata Core.<$> (Core.parseJSONObject o))
...
newAuditLog_Request 
    :: 
       ...
       -- ^  Properties of the object. See 'additional'.
    -> AuditLog_Request
newAuditLog_Request additional
  = AuditLog_Request{additional = additional}
instance Core.FromJSON AuditLog_Request where
        parseJSON
          = Core.withObject "AuditLog_Request"
              (\ o -> AuditLog_Request Core.<$> (Core.parseJSONObject o))
...
instance Core.FromJSON AuditLog_ResourceOriginalState where
        parseJSON
          = Core.withObject "AuditLog_ResourceOriginalState"
              (\ o ->
                 AuditLog_ResourceOriginalState Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON AuditLog_ResourceOriginalState where
        toJSON AuditLog_ResourceOriginalState{..} = Core.toJSON additional
...
newAuditLog_Response 
    :: 
       ...
       -- ^  Properties of the object. See 'additional'.
    -> AuditLog_Response
newAuditLog_Response additional
  = AuditLog_Response{additional = additional}
instance Core.FromJSON AuditLog_Response where
        parseJSON
          = Core.withObject "AuditLog_Response"
              (\ o -> AuditLog_Response Core.<$> (Core.parseJSONObject o))
...
instance Core.FromJSON AuditLog_ServiceData where
        parseJSON
          = Core.withObject "AuditLog_ServiceData"
              (\ o -> AuditLog_ServiceData Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON AuditLog_ServiceData where
        toJSON AuditLog_ServiceData{..} = Core.toJSON additional
...
newAuth 
    ::  Auth
newAuth
  = Auth{accessLevels = Core.Nothing, audiences = Core.Nothing,
         claims = Core.Nothing, presenter = Core.Nothing,
...
instance Core.FromJSON Auth where
        parseJSON
          = Core.withObject "Auth"
              (\ o ->
                 Auth Core.<$>
                   (o Core..:? "accessLevels") Core.<*> (o Core..:? "audiences")
                     Core.<*> (o Core..:? "claims")
...
instance Core.FromJSON Auth_Claims where
        parseJSON
          = Core.withObject "Auth_Claims"
              (\ o -> Auth_Claims Core.<$> (Core.parseJSONObject o))
...
newAuthenticationInfo 
    ::  AuthenticationInfo
newAuthenticationInfo
  = AuthenticationInfo{authoritySelector = Core.Nothing,
                       principalEmail = Core.Nothing, principalSubject = Core.Nothing,
...
instance Core.ToJSON AuthenticationInfo where
        toJSON AuthenticationInfo{..}
          = Core.object
              (Core.catMaybes
                 [("authoritySelector" Core..=) Core.<$> authoritySelector,
                  ("principalEmail" Core..=) Core.<$> principalEmail,
                  ("principalSubject" Core..=) Core.<$> principalSubject,
                  ("serviceAccountDelegationInfo" Core..=) Core.<$>
                    serviceAccountDelegationInfo,
                  ("serviceAccountKeyName" Core..=) Core.<$> serviceAccountKeyName,
                  ("serviceDelegationHistory" Core..=) Core.<$>
                    serviceDelegationHistory,
                  ("thirdPartyPrincipal" Core..=) Core.<$> thirdPartyPrincipal])
...
instance Core.FromJSON AuthenticationInfo_ThirdPartyPrincipal where
        parseJSON
          = Core.withObject "AuthenticationInfo_ThirdPartyPrincipal"
              (\ o ->
                 AuthenticationInfo_ThirdPartyPrincipal Core.<$>
                   (Core.parseJSONObject o))

instance Core.ToJSON AuthenticationInfo_ThirdPartyPrincipal where
...
newAuthorizationInfo 
    ::  AuthorizationInfo
newAuthorizationInfo
  = AuthorizationInfo{granted = Core.Nothing,
                      permission = Core.Nothing, permissionType = Core.Nothing,
                      resource = Core.Nothing, resourceAttributes = Core.Nothing}
instance Core.FromJSON AuthorizationInfo where
        parseJSON
          = Core.withObject "AuthorizationInfo"
              (\ o ->
                 AuthorizationInfo Core.<$>
                   (o Core..:? "granted") Core.<*> (o Core..:? "permission") Core.<*>
                     (o Core..:? "permissionType")
                     Core.<*> (o Core..:? "resource")
...
newCheckRequest 
    ::  CheckRequest
newCheckRequest
  = CheckRequest{attributes = Core.Nothing, flags = Core.Nothing,
                 resources = Core.Nothing, serviceConfigId = Core.Nothing}
instance Core.FromJSON CheckRequest where
        parseJSON
          = Core.withObject "CheckRequest"
              (\ o ->
                 CheckRequest Core.<$>
                   (o Core..:? "attributes") Core.<*> (o Core..:? "flags") Core.<*>
                     (o Core..:? "resources")
          ...
          = Core.object
              (Core.catMaybes
                 [("attributes" Core..=) Core.<$> attributes,
                  ...
                  ("serviceConfigId" Core..=) Core.<$> serviceConfigId])
...
newCheckResponse 
    ::  CheckResponse
newCheckResponse
  = CheckResponse{dynamicMetadata = Core.Nothing,
                  headers = Core.Nothing, status = Core.Nothing}
instance Core.FromJSON CheckResponse where
        parseJSON
          = Core.withObject "CheckResponse"
              (\ o ->
                 CheckResponse Core.<$>
                   (o Core..:? "dynamicMetadata") Core.<*> (o Core..:? "headers")
                     Core.<*> (o Core..:? "status"))
...
instance Core.FromJSON CheckResponse_DynamicMetadata where
        parseJSON
          = Core.withObject "CheckResponse_DynamicMetadata"
              (\ o ->
...
instance Core.FromJSON CheckResponse_Headers where
        parseJSON
          = Core.withObject "CheckResponse_Headers"
              (\ o -> CheckResponse_Headers Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON CheckResponse_Headers where
        toJSON CheckResponse_Headers{..} = Core.toJSON additional
...
newFirstPartyPrincipal 
    ::  FirstPartyPrincipal
newFirstPartyPrincipal
  = FirstPartyPrincipal{principalEmail = Core.Nothing,
                        serviceMetadata = Core.Nothing}
...
instance Core.ToJSON FirstPartyPrincipal where
        toJSON FirstPartyPrincipal{..}
          = Core.object
              (Core.catMaybes
                 [("principalEmail" Core..=) Core.<$> principalEmail,
                  ("serviceMetadata" Core..=) Core.<$> serviceMetadata])
...
instance Core.FromJSON FirstPartyPrincipal_ServiceMetadata where
        parseJSON
          = Core.withObject "FirstPartyPrincipal_ServiceMetadata"
              (\ o ->
                 FirstPartyPrincipal_ServiceMetadata Core.<$>
                   (Core.parseJSONObject o))

instance Core.ToJSON FirstPartyPrincipal_ServiceMetadata where
...
newOrgPolicyViolationInfo 
    ::  OrgPolicyViolationInfo
newOrgPolicyViolationInfo
  = OrgPolicyViolationInfo{payload = Core.Nothing,
                           resourceTags = Core.Nothing, resourceType = Core.Nothing,
          ...
          = Core.withObject "OrgPolicyViolationInfo"
              (\ o ->
                 OrgPolicyViolationInfo Core.<$>
                   (o Core..:? "payload") Core.<*> (o Core..:? "resourceTags")
...
instance Core.FromJSON OrgPolicyViolationInfo_Payload where
        parseJSON
          = Core.withObject "OrgPolicyViolationInfo_Payload"
              (\ o ->
                 OrgPolicyViolationInfo_Payload Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON OrgPolicyViolationInfo_Payload where
        toJSON OrgPolicyViolationInfo_Payload{..} = Core.toJSON additional
...
instance Core.FromJSON OrgPolicyViolationInfo_ResourceTags where
        parseJSON
          = Core.withObject "OrgPolicyViolationInfo_ResourceTags"
              (\ o ->
                 OrgPolicyViolationInfo_ResourceTags Core.<$>
                   (Core.parseJSONObject o))

instance Core.ToJSON OrgPolicyViolationInfo_ResourceTags where
...
newPeer 
    ::  Peer
newPeer
  = Peer{ip = Core.Nothing, labels = Core.Nothing,
         port = Core.Nothing, principal = Core.Nothing,
...
instance Core.FromJSON Peer where
        parseJSON
          = Core.withObject "Peer"
              (\ o ->
                 Peer Core.<$>
                   (o Core..:? "ip") Core.<*> (o Core..:? "labels") Core.<*>
                     (o Core..:? "port" Core.<&> Core.fmap Core.fromAsText)
                     Core.<*> (o Core..:? "principal")
...
instance Core.ToJSON Peer where
        toJSON Peer{..}
          = Core.object
              (Core.catMaybes
                 [("ip" Core..=) Core.<$> ip, ("labels" Core..=) Core.<$> labels,
...
instance Core.FromJSON Peer_Labels where
        parseJSON
          = Core.withObject "Peer_Labels"
              (\ o -> Peer_Labels Core.<$> (Core.parseJSONObject o))
...
instance Core.FromJSON PolicyViolationInfo where
        parseJSON
          = Core.withObject "PolicyViolationInfo"
              (\ o ->
                 PolicyViolationInfo Core.<$> (o Core..:? "orgPolicyViolationInfo"))
...
newReportRequest 
    ::  ReportRequest
newReportRequest
  = ReportRequest{operations = Core.Nothing,
...
instance Core.FromJSON ReportRequest where
        parseJSON
          = Core.withObject "ReportRequest"
              (\ o ->
                 ReportRequest Core.<$>
                   (o Core..:? "operations") Core.<*> (o Core..:? "serviceConfigId"))
...
instance Core.FromJSON ReportResponse where
        parseJSON
          = Core.withObject "ReportResponse"
              (\ o -> ReportResponse Core.<$> (o Core..:? "extensions"))

instance Core.ToJSON ReportResponse where
        toJSON ReportResponse{..}
          = Core.object
              (Core.catMaybes [("extensions" Core..=) Core.<$> extensions])
...
instance Core.FromJSON ReportResponse_Extensions where
        parseJSON
          = Core.withObject "ReportResponse_Extensions"
              (\ o ->
                 ReportResponse_Extensions Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON ReportResponse_Extensions where
        toJSON ReportResponse_Extensions{..} = Core.toJSON additional
...
newRequest 
    ::  Request'
newRequest
  = Request'{auth = Core.Nothing, headers = Core.Nothing,
             host = Core.Nothing, id = Core.Nothing, method = Core.Nothing,
             origin = Core.Nothing, path = Core.Nothing,
             protocol = Core.Nothing, query = Core.Nothing,
             reason = Core.Nothing, scheme = Core.Nothing, size = Core.Nothing,
             time = Core.Nothing}
instance Core.FromJSON Request' where
        parseJSON
          = Core.withObject "Request'"
              (\ o ->
                 Request' Core.<$>
                   (o Core..:? "auth") Core.<*> (o Core..:? "headers") Core.<*>
                     (o Core..:? "host")
                     Core.<*> (o Core..:? "id")
                     Core.<*> (o Core..:? "method")
                     Core.<*> (o Core..:? "origin")
        ...
        toJSON Request'{..}
          = Core.object
              (Core.catMaybes
                 [("auth" Core..=) Core.<$> auth,
                  ("headers" Core..=) Core.<$> headers,
                  ("host" Core..=) Core.<$> host, ("id" Core..=) Core.<$> id,
                  ("method" Core..=) Core.<$> method,
                  ("origin" Core..=) Core.<$> origin, ("path" Core..=) Core.<$> path,
                  ("protocol" Core..=) Core.<$> protocol,
...
newRequest_Headers 
    :: 
       ...
       -- ^  See 'additional'.
    -> Request_Headers
newRequest_Headers additional
  = Request_Headers{additional = additional}
instance Core.FromJSON Request_Headers where
        parseJSON
          = Core.withObject "Request_Headers"
              (\ o -> Request_Headers Core.<$> (Core.parseJSONObject o))
...
newRequestMetadata 
    ::  RequestMetadata
newRequestMetadata
  = RequestMetadata{callerIp = Core.Nothing,
          ...
          = Core.withObject "RequestMetadata"
              (\ o ->
                 RequestMetadata Core.<$>
                   (o Core..:? "callerIp") Core.<*> (o Core..:? "callerNetwork")
          ...
          = Core.object
              (Core.catMaybes
                 [("callerIp" Core..=) Core.<$> callerIp,
                  ...
                  ("destinationAttributes" Core..=) Core.<$> destinationAttributes,
...
newResource 
    ::  Resource
newResource
  = Resource{annotations = Core.Nothing, createTime = Core.Nothing,
             deleteTime = Core.Nothing, displayName = Core.Nothing,
             etag = Core.Nothing, labels = Core.Nothing,
             location = Core.Nothing, name = Core.Nothing,
             service = Core.Nothing, type' = Core.Nothing, uid = Core.Nothing,
          ...
          = Core.object
              (Core.catMaybes
                 [("annotations" Core..=) Core.<$> annotations,
                  ...
                  ("name" Core..=) Core.<$> name,
                  ("service" Core..=) Core.<$> service,
                  ("type" Core..=) Core.<$> type', ("uid" Core..=) Core.<$> uid,
...
instance Core.FromJSON Resource_Annotations where
        parseJSON
          = Core.withObject "Resource_Annotations"
              (\ o -> Resource_Annotations Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON Resource_Annotations where
        toJSON Resource_Annotations{..} = Core.toJSON additional
...
newResourceInfo 
    ::  ResourceInfo
newResourceInfo
  = ResourceInfo{container = Core.Nothing, location = Core.Nothing,
                 name = Core.Nothing, permission = Core.Nothing,
                 type' = Core.Nothing}
instance Core.FromJSON ResourceInfo where
        parseJSON
          = Core.withObject "ResourceInfo"
              (\ o ->
                 ResourceInfo Core.<$>
                   (o Core..:? "container") Core.<*> (o Core..:? "location") Core.<*>
                     (o Core..:? "name")
...
newResourceLocation 
    ::  ResourceLocation
newResourceLocation
  = ResourceLocation{currentLocations = Core.Nothing,
                     originalLocations = Core.Nothing}
...
instance Core.ToJSON ResourceLocation where
        toJSON ResourceLocation{..}
          = Core.object
              (Core.catMaybes
                 [("currentLocations" Core..=) Core.<$> currentLocations,
                  ("originalLocations" Core..=) Core.<$> originalLocations])
...
newResponse 
    ::  Response
newResponse
  = Response{backendLatency = Core.Nothing, code = Core.Nothing,
             headers = Core.Nothing, size = Core.Nothing, time = Core.Nothing}
              ...
              (\ o ->
                 Response Core.<$>
                   (o Core..:? "backendLatency") Core.<*>
                     (o Core..:? "code" Core.<&> Core.fmap Core.fromAsText)
                     Core.<*> (o Core..:? "headers")
                     Core.<*> (o Core..:? "size" Core.<&> Core.fmap Core.fromAsText)
                     Core.<*> (o Core..:? "time"))
...
newResponse_Headers 
    :: 
       ...
       -- ^  See 'additional'.
    -> Response_Headers
newResponse_Headers additional
  = Response_Headers{additional = additional}
instance Core.FromJSON Response_Headers where
        parseJSON
          = Core.withObject "Response_Headers"
              (\ o -> Response_Headers Core.<$> (Core.parseJSONObject o))
...
newServiceAccountDelegationInfo 
    ::  ServiceAccountDelegationInfo
newServiceAccountDelegationInfo
  = ServiceAccountDelegationInfo{firstPartyPrincipal = Core.Nothing,
                                 principalSubject = Core.Nothing,
                                 thirdPartyPrincipal = Core.Nothing}
instance Core.FromJSON ServiceAccountDelegationInfo where
        parseJSON
...
instance Core.ToJSON ServiceAccountDelegationInfo where
        toJSON ServiceAccountDelegationInfo{..}
          = Core.object
              (Core.catMaybes
                 [("firstPartyPrincipal" Core..=) Core.<$> firstPartyPrincipal,
                  ("principalSubject" Core..=) Core.<$> principalSubject,
                  ("thirdPartyPrincipal" Core..=) Core.<$> thirdPartyPrincipal])
...
newServiceDelegationHistory 
    ::  ServiceDelegationHistory
newServiceDelegationHistory
  = ServiceDelegationHistory{originalPrincipal = Core.Nothing,
                             serviceMetadata = Core.Nothing}
...
instance Core.ToJSON ServiceDelegationHistory where
        toJSON ServiceDelegationHistory{..}
          = Core.object
              (Core.catMaybes
                 [("originalPrincipal" Core..=) Core.<$> originalPrincipal,
                  ("serviceMetadata" Core..=) Core.<$> serviceMetadata])
...
newServiceMetadata 
    ::  ServiceMetadata
newServiceMetadata
  = ServiceMetadata{jobMetadata = Core.Nothing,
                    principalSubject = Core.Nothing, serviceDomain = Core.Nothing}
instance Core.FromJSON ServiceMetadata where
        parseJSON
          = Core.withObject "ServiceMetadata"
              (\ o ->
                 ServiceMetadata Core.<$>
                   (o Core..:? "jobMetadata") Core.<*> (o Core..:? "principalSubject")
...
instance Core.FromJSON ServiceMetadata_JobMetadata where
        parseJSON
          = Core.withObject "ServiceMetadata_JobMetadata"
              (\ o ->
                 ServiceMetadata_JobMetadata Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON ServiceMetadata_JobMetadata where
        toJSON ServiceMetadata_JobMetadata{..} = Core.toJSON additional
...
instance Core.FromJSON ThirdPartyPrincipal_ThirdPartyClaims where
        parseJSON
          = Core.withObject "ThirdPartyPrincipal_ThirdPartyClaims"
              (\ o ->
                 ThirdPartyPrincipal_ThirdPartyClaims Core.<$>
                   (Core.parseJSONObject o))

instance Core.ToJSON ThirdPartyPrincipal_ThirdPartyClaims where
...
newV2HttpRequest 
    ::  V2HttpRequest
newV2HttpRequest
  = V2HttpRequest{cacheFillBytes = Core.Nothing,
...
newV2LogEntry 
    ::  V2LogEntry
newV2LogEntry
  = V2LogEntry{httpRequest = Core.Nothing, insertId = Core.Nothing,
               labels = Core.Nothing, monitoredResourceLabels = Core.Nothing,
               name = Core.Nothing, operation = Core.Nothing,
               protoPayload = Core.Nothing, severity = Core.Nothing,
               sourceLocation = Core.Nothing, structPayload = Core.Nothing,
               textPayload = Core.Nothing, timestamp = Core.Nothing,
               trace = Core.Nothing}
instance Core.FromJSON V2LogEntry where
        parseJSON
          = Core.withObject "V2LogEntry"
              (\ o ->
                 V2LogEntry Core.<$>
                   (o Core..:? "httpRequest") Core.<*> (o Core..:? "insertId")
                     Core.<*> (o Core..:? "labels")
...
newV2LogEntry_Labels 
    :: 
       ...
       -- ^  See 'additional'.
    -> V2LogEntry_Labels
newV2LogEntry_Labels additional
  = V2LogEntry_Labels{additional = additional}
instance Core.FromJSON V2LogEntry_Labels where
        parseJSON
          = Core.withObject "V2LogEntry_Labels"
              (\ o -> V2LogEntry_Labels Core.<$> (Core.parseJSONObject o))
...
instance Core.FromJSON V2LogEntry_MonitoredResourceLabels where
        parseJSON
          = Core.withObject "V2LogEntry_MonitoredResourceLabels"
              (\ o ->
                 V2LogEntry_MonitoredResourceLabels Core.<$>
                   (Core.parseJSONObject o))

instance Core.ToJSON V2LogEntry_MonitoredResourceLabels where
...
instance Core.FromJSON V2LogEntry_ProtoPayload where
        parseJSON
          = Core.withObject "V2LogEntry_ProtoPayload"
              (\ o -> V2LogEntry_ProtoPayload Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON V2LogEntry_ProtoPayload where
        toJSON V2LogEntry_ProtoPayload{..} = Core.toJSON additional
...
instance Core.FromJSON V2LogEntry_StructPayload where
        parseJSON
          = Core.withObject "V2LogEntry_StructPayload"
              (\ o -> V2LogEntry_StructPayload Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON V2LogEntry_StructPayload where
        toJSON V2LogEntry_StructPayload{..} = Core.toJSON additional
...
newV2LogEntryOperation 
    ::  V2LogEntryOperation
newV2LogEntryOperation
  = V2LogEntryOperation{first = Core.Nothing, id = Core.Nothing,
...
newV2LogEntrySourceLocation 
    ::  V2LogEntrySourceLocation
newV2LogEntrySourceLocation
  = V2LogEntrySourceLocation{file = Core.Nothing,
...
newV2ResourceEvent 
    ::  V2ResourceEvent
newV2ResourceEvent
  = V2ResourceEvent{contextId = Core.Nothing,
                    destinations = Core.Nothing, parent = Core.Nothing,
                    path = Core.Nothing, payload = Core.Nothing,
                    resource = Core.Nothing, type' = Core.Nothing}
instance Core.FromJSON V2ResourceEvent where
        parseJSON
          = Core.withObject "V2ResourceEvent"
              (\ o ->
                 V2ResourceEvent Core.<$>
                   (o Core..:? "contextId" Core.<&> Core.fmap Core.fromAsText)
...
instance Core.ToJSON V2ResourceEvent where
        toJSON V2ResourceEvent{..}
          = Core.object
              (Core.catMaybes
                 [("contextId" Core..=) Core.. Core.AsText Core.<$> contextId,
                  ("destinations" Core..=) Core.<$> destinations,
                  ("parent" Core..=) Core.<$> parent, ("path" Core..=) Core.<$> path,
                  ("payload" Core..=) Core.<$> payload,
                  ("resource" Core..=) Core.<$> resource,
                  ("type" Core..=) Core.<$> type'])
...
instance Core.FromJSON V2ResourceEvent_Payload where
        parseJSON
          = Core.withObject "V2ResourceEvent_Payload"
              (\ o -> V2ResourceEvent_Payload Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON V2ResourceEvent_Payload where
        toJSON V2ResourceEvent_Payload{..} = Core.toJSON additional
...
newViolationInfo 
    ::  ViolationInfo
newViolationInfo
  = ViolationInfo{checkedValue = Core.Nothing,
                  constraint = Core.Nothing, errorMessage = Core.Nothing,
...
instance Core.FromJSON ViolationInfo where
        parseJSON
          = Core.withObject "ViolationInfo"
              (\ o ->
                 ViolationInfo Core.<$>
                   (o Core..:? "checkedValue") Core.<*> (o Core..:? "constraint")
