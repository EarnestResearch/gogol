...
type ServiceControlServicesReportResource =
     "v2" Core.:>
       "services" Core.:>
         Core.CaptureMode "serviceName" "report" Core.Text Core.:>
                   ...
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.ReqBody '[Core.JSON] ReportRequest Core.:>
                         Core.Post '[Core.JSON] ReportResponse
...
newServiceControlServicesReport payload serviceName
  = ServiceControlServicesReport{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 payload = payload, serviceName = serviceName,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ServiceControlServicesReport where
        type Rs ServiceControlServicesReport = ReportResponse
        type Scopes ServiceControlServicesReport =
             '[CloudPlatform'FullControl, Servicecontrol'FullControl]
        requestClient ServiceControlServicesReport{..}
          = go serviceName xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ServiceControlServicesReportResource)
                      Core.mempty
