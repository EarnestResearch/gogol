...
newSpannerProjectsInstancesBackupsPatch name payload
  = SpannerProjectsInstancesBackupsPatch{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         name = name, payload = payload, updateMask = Core.Nothing,
                                         uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SpannerProjectsInstancesBackupsPatch
         where
        type Rs SpannerProjectsInstancesBackupsPatch = Backup
        type Scopes SpannerProjectsInstancesBackupsPatch =
             '[CloudPlatform'FullControl, Spanner'Admin]
        requestClient SpannerProjectsInstancesBackupsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy SpannerProjectsInstancesBackupsPatchResource)
                      Core.mempty
