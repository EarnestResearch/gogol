...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @reports.activities.watch@.
module Gogol.Admin.Reports.Activities.Watch
    (
    -- * Resource
           ...
           "activity" Core.:>
             "users" Core.:>
               Core.Capture "userKey" Core.Text Core.:>
                 "applications" Core.:>
                   Core.Capture "applicationName" ActivitiesWatchApplicationName
                     Core.:>
                     "watch" Core.:>
                       Core.QueryParam "$.xgafv" Xgafv Core.:>
                         Core.QueryParam "access_token" Core.Text Core.:>
                           Core.QueryParam "actorIpAddress" Core.Text Core.:>
                             Core.QueryParam "callback" Core.Text Core.:>
                               Core.QueryParam "customerId" Core.Text Core.:>
                                 Core.QueryParam "endTime" Core.Text Core.:>
                                   Core.QueryParam "eventName" Core.Text Core.:>
...
newReportsActivitiesWatch applicationName payload userKey
  = ReportsActivitiesWatch{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, actorIpAddress = Core.Nothing,
                           applicationName = applicationName, callback = Core.Nothing,
                           customerId = Core.Nothing, endTime = Core.Nothing,
                           eventName = Core.Nothing, filters = Core.Nothing,
                           groupIdFilter = Core.Nothing, maxResults = 1000, orgUnitID = "",
                           pageToken = Core.Nothing, payload = payload,
                           startTime = Core.Nothing, uploadType = Core.Nothing,
                           uploadProtocol = Core.Nothing, userKey = userKey}
instance Core.GoogleRequest ReportsActivitiesWatch where
        type Rs ReportsActivitiesWatch = Channel
        type Scopes ReportsActivitiesWatch =
             '[Admin'Reports'Audit'Readonly]
        requestClient ReportsActivitiesWatch{..}
          = go userKey applicationName xgafv accessToken actorIpAddress
              callback
              customerId
              endTime
              eventName
