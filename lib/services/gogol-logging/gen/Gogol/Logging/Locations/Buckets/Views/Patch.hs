...
-- |
-- Module      : Gogol.Logging.Locations.Buckets.Views.Patch
...
-- 'LoggingLocationsBucketsViewsPatch' request conforms to.
type LoggingLocationsBucketsViewsPatchResource =
     "v2" Core.:>
...
-- | Updates a view on a log bucket. This method replaces the following fields in the existing view with values from the new view: filter. If an UNAVAILABLE error is returned, this indicates that system is not in a state where it can update the view. If this occurs, please try again in a few minutes.
--
-- /See:/ 'newLoggingLocationsBucketsViewsPatch' smart constructor.
data LoggingLocationsBucketsViewsPatch = LoggingLocationsBucketsViewsPatch
    {
      -- | V1 error format.
...
-- | Creates a value of 'LoggingLocationsBucketsViewsPatch' with the minimum fields required to make a request.
newLoggingLocationsBucketsViewsPatch 
    ::  Core.Text
    ...
    -> LoggingLocationsBucketsViewsPatch
newLoggingLocationsBucketsViewsPatch name payload =
  LoggingLocationsBucketsViewsPatch
    { xgafv = Core.Nothing
...
instance Core.GoogleRequest
           LoggingLocationsBucketsViewsPatch
         where
        type Rs LoggingLocationsBucketsViewsPatch = LogView
        type Scopes LoggingLocationsBucketsViewsPatch =
             '[CloudPlatform'FullControl, Logging'Admin]
        requestClient LoggingLocationsBucketsViewsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy LoggingLocationsBucketsViewsPatchResource)
                      Core.mempty
