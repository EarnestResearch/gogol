...
-- |
-- Module      : Gogol.Logging.Projects.Sinks.Create
...
-- 'LoggingProjectsSinksCreate' request conforms to.
type LoggingProjectsSinksCreateResource =
     "v2" Core.:>
...
-- | Creates a sink that exports specified log entries to a destination. The export of newly-ingested log entries begins immediately, unless the sink\'s writer_identity is not permitted to write to the destination. A sink can export log entries only from the resource owning the sink.
--
-- /See:/ 'newLoggingProjectsSinksCreate' smart constructor.
data LoggingProjectsSinksCreate = LoggingProjectsSinksCreate
    {
      -- | V1 error format.
...
-- | Creates a value of 'LoggingProjectsSinksCreate' with the minimum fields required to make a request.
newLoggingProjectsSinksCreate 
    ::  Core.Text
    ...
    -> LoggingProjectsSinksCreate
newLoggingProjectsSinksCreate parent payload =
  LoggingProjectsSinksCreate
    { xgafv = Core.Nothing
...
instance Core.GoogleRequest
           LoggingProjectsSinksCreate
         where
        type Rs LoggingProjectsSinksCreate = LogSink
        type Scopes LoggingProjectsSinksCreate =
             '[CloudPlatform'FullControl, Logging'Admin]
        requestClient LoggingProjectsSinksCreate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy LoggingProjectsSinksCreateResource)
                      Core.mempty
