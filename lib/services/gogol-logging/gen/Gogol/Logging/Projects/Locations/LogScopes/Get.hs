...
newLoggingProjectsLocationsLogScopesGet name
  = LoggingProjectsLocationsLogScopesGet{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         name = name, uploadType = Core.Nothing,
                                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest LoggingProjectsLocationsLogScopesGet
         where
        type Rs LoggingProjectsLocationsLogScopesGet = LogScope
        type Scopes LoggingProjectsLocationsLogScopesGet =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly, Logging'Admin,
               Logging'Read]
        requestClient LoggingProjectsLocationsLogScopesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy LoggingProjectsLocationsLogScopesGetResource)
                      Core.mempty
