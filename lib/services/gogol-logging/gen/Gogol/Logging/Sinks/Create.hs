...
-- |
-- Module      : Gogol.Logging.Sinks.Create
...
-- 'LoggingSinksCreate' request conforms to.
type LoggingSinksCreateResource =
     "v2" Core.:>
...
-- | Creates a sink that exports specified log entries to a destination. The export of newly-ingested log entries begins immediately, unless the sink\'s writer_identity is not permitted to write to the destination. A sink can export log entries only from the resource owning the sink.
--
-- /See:/ 'newLoggingSinksCreate' smart constructor.
data LoggingSinksCreate = LoggingSinksCreate
    {
      -- | V1 error format.
...
-- | Creates a value of 'LoggingSinksCreate' with the minimum fields required to make a request.
newLoggingSinksCreate 
    ::  Core.Text
    ...
    -> LoggingSinksCreate
newLoggingSinksCreate parent payload =
  LoggingSinksCreate
    { xgafv = Core.Nothing
...
instance Core.GoogleRequest LoggingSinksCreate where
        type Rs LoggingSinksCreate = LogSink
        type Scopes LoggingSinksCreate =
             '[CloudPlatform'FullControl, Logging'Admin]
        requestClient LoggingSinksCreate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy LoggingSinksCreateResource)
                      Core.mempty
