...
--
-- Lists messages in a space that the caller is a member of, including messages from blocked members and spaces. If you list messages from a space with no messages, the response is an empty object. When using a REST\/HTTP interface, the response contains an empty JSON object, @{}@. For an example, see <https://developers.google.com/workspace/chat/api/guides/v1/messages/list List messages>. Requires <https://developers.google.com/workspace/chat/authenticate-authorize-chat-user user authentication> with one of the following <https://developers.google.com/workspace/chat/authenticate-authorize#chat-api-scopes authorization scopes>: - @https:\/\/www.googleapis.com\/auth\/chat.messages.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.messages@ - @https:\/\/www.googleapis.com\/auth\/chat.import@ (import mode spaces only)
--
