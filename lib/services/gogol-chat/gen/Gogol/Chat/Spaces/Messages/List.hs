...
--
-- <https://developers.google.com/workspace/preview Developer Preview>: Lists messages in a space that the caller is a member of, including messages from blocked members and spaces. Requires <https://developers.google.com/chat/api/guides/auth/users user authentication> and the @chat.messages@ or @chat.messages.readonly@ authorization scope. This method is only supported in spaces that don\'t allow users from outside the Workspace organization to join.
--
...
instance Core.GoogleRequest ChatSpacesMessagesList
         where
        type Rs ChatSpacesMessagesList = ListMessagesResponse
        type Scopes ChatSpacesMessagesList =
             '[Chat'Messages, Chat'Messages'Readonly]
        requestClient ChatSpacesMessagesList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ChatSpacesMessagesListResource)
                      Core.mempty
