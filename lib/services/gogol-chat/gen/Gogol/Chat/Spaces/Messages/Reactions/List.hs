...
--
-- <https://developers.google.com/workspace/preview Developer Preview>: Lists reactions to a message. Requires <https://developers.google.com/chat/api/guides/auth/users user authentication> and @chat.messages@, @chat.messages.readonly@, @chat.messages.reactions@, or @chat.messages.reactions.readonly@ scope.
--
...
data ChatSpacesMessagesReactionsList = ChatSpacesMessagesReactionsList
    ...
    , callback :: (Core.Maybe Core.Text)
      -- | Optional. A query filter. You can filter reactions by <https://developers.google.com/chat/api/reference/rest/v1/Emoji emoji> (either @emoji.unicode@ or @emoji.custom_emoji.uid@) and <https://developers.google.com/chat/api/reference/rest/v1/User user> (@user.name@). To filter reactions for multiple emojis or users, join similar fields with the @OR@ operator, such as @emoji.unicode = \"🙂\" OR emoji.unicode = \"👍\"@ and @user.name = \"users\/AAAAAA\" OR user.name = \"users\/BBBBBB\"@. To filter reactions by emoji and user, use the @AND@ operator, such as @emoji.unicode = \"🙂\" AND user.name = \"users\/AAAAAA\"@. If your query uses both @AND@ and @OR@, group them with parentheses. For example, the following queries are valid:
