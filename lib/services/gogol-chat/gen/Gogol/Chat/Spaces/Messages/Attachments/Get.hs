...
--
-- Gets the metadata of a message attachment. The attachment data is fetched using the <https://developers.google.com/workspace/chat/api/reference/rest/v1/media/download media API>. For an example, see <https://developers.google.com/workspace/chat/get-media-attachments Get metadata about a message attachment>. Requires <https://developers.google.com/workspace/chat/authenticate-authorize-chat-app app authentication> with the <https://developers.google.com/workspace/chat/authenticate-authorize#chat-api-scopes authorization scope>: - @https:\/\/www.googleapis.com\/auth\/chat.bot@
--
...
newChatSpacesMessagesAttachmentsGet name
  = ChatSpacesMessagesAttachmentsGet{xgafv = Core.Nothing,
