...
-- only use this method to set up a DM with the calling app. To add the calling app as a member of a space or an existing DM between two human users, see <https://developers.google.com/workspace/chat/create-members Invite or add a user or app to a space>. If a DM already exists between two users, even when one user blocks the other at the time a request is made, then the existing DM is returned. Spaces with threaded replies aren\'t supported. If you receive the error message @ALREADY_EXISTS@ when setting up a space, try a different @displayName@. An existing space within the Google Workspace organization might already use this display name. Requires <https://developers.google.com/workspace/chat/authenticate-authorize-chat-user user authentication> with one of the following <https://developers.google.com/workspace/chat/authenticate-authorize#chat-api-scopes authorization scopes>: - @https:\/\/www.googleapis.com\/auth\/chat.spaces.create@ - @https:\/\/www.googleapis.com\/auth\/chat.spaces@
--
...
newChatSpacesSetup payload
  = ChatSpacesSetup{xgafv = Core.Nothing, accessToken = Core.Nothing,
...
instance Core.GoogleRequest ChatSpacesSetup where
        ...
        type Scopes ChatSpacesSetup = '[Chat'Spaces, Chat'Spaces'Create]
