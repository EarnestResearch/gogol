...
--
-- Returns a list of spaces in a Google Workspace organization based on an administrator\'s search. Requires <https://developers.google.com/workspace/chat/authenticate-authorize-chat-user#admin-privileges user authentication with administrator privileges> and one of the following <https://developers.google.com/workspace/chat/authenticate-authorize#chat-api-scopes authorization scopes>: - @https:\/\/www.googleapis.com\/auth\/chat.admin.spaces.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.admin.spaces@ In the request, set @use_admin_access@ to @true@.
--
...
newChatSpacesSearch 
    ::  ChatSpacesSearch
newChatSpacesSearch
  = ChatSpacesSearch{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ChatSpacesSearch where
        ...
        requestClient ChatSpacesSearch{..}
          = go xgafv accessToken callback orderBy pageSize pageToken query
              uploadType
              uploadProtocol
              useAdminAccess
