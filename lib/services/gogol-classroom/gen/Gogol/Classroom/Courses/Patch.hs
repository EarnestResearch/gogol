...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.patch@.
module Gogol.Classroom.Courses.Patch
    (
    -- * Resource
...
newClassroomCoursesPatch id payload
  = ClassroomCoursesPatch{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ClassroomCoursesPatch where
        type Rs ClassroomCoursesPatch = Course
        type Scopes ClassroomCoursesPatch = '[Classroom'Courses]
        requestClient ClassroomCoursesPatch{..}
          = go id xgafv accessToken callback updateMask uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ClassroomCoursesPatchResource)
                      Core.mempty
