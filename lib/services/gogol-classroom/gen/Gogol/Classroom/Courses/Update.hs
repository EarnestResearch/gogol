...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.update@.
module Gogol.Classroom.Courses.Update
    (
    -- * Resource
...
newClassroomCoursesUpdate id payload
  = ClassroomCoursesUpdate{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing, id = id,
                           payload = payload, uploadType = Core.Nothing,
                           uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ClassroomCoursesUpdate where
        type Rs ClassroomCoursesUpdate = Course
        type Scopes ClassroomCoursesUpdate = '[Classroom'Courses]
        requestClient ClassroomCoursesUpdate{..}
          = go id xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              classroomService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ClassroomCoursesUpdateResource)
                      Core.mempty
