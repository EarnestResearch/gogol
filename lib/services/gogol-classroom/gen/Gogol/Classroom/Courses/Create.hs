...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.create@.
module Gogol.Classroom.Courses.Create
    (
    -- * Resource
...
newClassroomCoursesCreate payload
  = ClassroomCoursesCreate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ClassroomCoursesCreate where
        ...
        type Scopes ClassroomCoursesCreate = '[Classroom'Courses]
        requestClient ClassroomCoursesCreate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ClassroomCoursesCreateResource)
                      Core.mempty
