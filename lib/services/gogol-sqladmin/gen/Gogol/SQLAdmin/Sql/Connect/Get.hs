                         ...
                         Core.QueryParam "uploadType" Core.Text Core.:>
                           Core.QueryParam "upload_protocol" Core.Text Core.:>
                             Core.QueryParam "alt" Core.AltJSON Core.:>
                               Core.Get '[Core.JSON] ConnectSettings
...
newSqlConnectGet instance' project
  = SqlConnectGet{xgafv = Core.Nothing, accessToken = Core.Nothing,
                  callback = Core.Nothing, instance' = instance', project = project,
                  readTime = Core.Nothing, uploadType = Core.Nothing,
                  uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SqlConnectGet where
        type Rs SqlConnectGet = ConnectSettings
        type Scopes SqlConnectGet =
             '[CloudPlatform'FullControl, Sqlservice'Admin]
        requestClient SqlConnectGet{..}
          = go project instance' xgafv accessToken callback readTime
              uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              sQLAdminService
          where go
                  = Core.buildClient (Core.Proxy :: Core.Proxy SqlConnectGetResource)
                      Core.mempty
