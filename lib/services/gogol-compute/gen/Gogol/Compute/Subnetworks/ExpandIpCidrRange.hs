                               ...
                               Core.QueryParam "uploadType" Core.Text Core.:>
                                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                                   Core.QueryParam "alt" Core.AltJSON Core.:>
                                     Core.ReqBody '[Core.JSON] SubnetworksExpandIpCidrRangeRequest
...
newComputeSubnetworksExpandIpCidrRange 
    ...
    -> ComputeSubnetworksExpandIpCidrRange
newComputeSubnetworksExpandIpCidrRange payload project region
  subnetwork
  = ComputeSubnetworksExpandIpCidrRange{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        payload = payload, project = project, region = region,
                                        requestId = Core.Nothing, subnetwork = subnetwork,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ComputeSubnetworksExpandIpCidrRange
         where
        type Rs ComputeSubnetworksExpandIpCidrRange = Operation
        type Scopes ComputeSubnetworksExpandIpCidrRange =
             '[CloudPlatform'FullControl, Compute'FullControl]
        requestClient ComputeSubnetworksExpandIpCidrRange{..}
          = go project region subnetwork xgafv accessToken callback requestId
              uploadType
              uploadProtocol
