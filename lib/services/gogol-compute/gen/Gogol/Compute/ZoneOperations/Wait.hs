...
instance Core.GoogleRequest ComputeZoneOperationsWait
         where
        type Rs ComputeZoneOperationsWait = Operation
        type Scopes ComputeZoneOperationsWait =
             '[CloudPlatform'FullControl, Compute'FullControl,
               Compute'Readonly]
        requestClient ComputeZoneOperationsWait{..}
          = go project zone operation xgafv accessToken
              callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ComputeZoneOperationsWaitResource)
                      Core.mempty
