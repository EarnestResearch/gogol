...
-- |
-- Module      : Gogol.DLP.Projects.Locations.Image.Redact
...
-- 'DLPProjectsLocationsImageRedact' request conforms to.
type DLPProjectsLocationsImageRedactResource =
     "v2" Core.:>
...
-- | Redacts potentially sensitive info from an image. This method has limits on input size, processing time, and output size. See https:\/\/cloud.google.com\/dlp\/docs\/redacting-sensitive-data-images to learn more. When no InfoTypes or CustomInfoTypes are specified in this request, the system will automatically choose what detectors to run. By default this may be all types, but may change over time as detectors are updated.
--
-- /See:/ 'newDLPProjectsLocationsImageRedact' smart constructor.
data DLPProjectsLocationsImageRedact = DLPProjectsLocationsImageRedact
    {
      -- | V1 error format.
...
-- | Creates a value of 'DLPProjectsLocationsImageRedact' with the minimum fields required to make a request.
newDLPProjectsLocationsImageRedact 
    ::  Core.Text
    ...
    -> DLPProjectsLocationsImageRedact
newDLPProjectsLocationsImageRedact parent payload =
  DLPProjectsLocationsImageRedact
    { xgafv = Core.Nothing
...
instance Core.GoogleRequest
           DLPProjectsLocationsImageRedact
         where
        type Rs DLPProjectsLocationsImageRedact =
             GooglePrivacyDlpV2RedactImageResponse
        type Scopes DLPProjectsLocationsImageRedact =
        ...
        requestClient DLPProjectsLocationsImageRedact{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DLPProjectsLocationsImageRedactResource)
                      Core.mempty
