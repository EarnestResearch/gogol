...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.users.watch@.
module Gogol.Admin.Directory.Users.Watch
    (
    -- * Resource
                         ...
                         Core.QueryParam "domain" Core.Text Core.:>
                           Core.QueryParam "event" UsersWatchEvent Core.:>
                             Core.QueryParam "maxResults" Core.Int32 Core.:>
                               Core.QueryParam "orderBy" UsersWatchOrderBy Core.:>
                                 Core.QueryParam "pageToken" Core.Text Core.:>
                                   Core.QueryParam "projection" UsersWatchProjection Core.:>
                                     Core.QueryParam "query" Core.Text Core.:>
                                       Core.QueryParam "showDeleted" Core.Text Core.:>
                                         Core.QueryParam "sortOrder" UsersWatchSortOrder Core.:>
                                           Core.QueryParam "uploadType" Core.Text Core.:>
                                             Core.QueryParam "upload_protocol" Core.Text Core.:>
                                               Core.QueryParam "viewType" UsersWatchViewType Core.:>
...
newDirectoryUsersWatch payload
  = DirectoryUsersWatch{xgafv = Core.Nothing,
                        accessToken = Core.Nothing, callback = Core.Nothing,
                        customFieldMask = Core.Nothing, customer = Core.Nothing,
                        domain = Core.Nothing, event = Core.Nothing, maxResults = 100,
                        orderBy = Core.Nothing, pageToken = Core.Nothing,
                        payload = payload, projection = UsersWatchProjection_Basic,
                        query = Core.Nothing, showDeleted = Core.Nothing,
                        sortOrder = Core.Nothing, uploadType = Core.Nothing,
...
instance Core.GoogleRequest DirectoryUsersWatch where
        ...
        requestClient DirectoryUsersWatch{..}
          = go xgafv accessToken callback customFieldMask customer domain
              event
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryUsersWatchResource)
                      Core.mempty
