...
--
-- DeleteSeries deletes all instances within the given study and series. Delete requests are equivalent to the GET requests specified in the Retrieve transaction. The method returns an Operation which will be marked successful when the deletion is complete. Warning: Instances cannot be inserted into a series that is being deleted by an operation until the operation completes. For samples that show how to call DeleteSeries, see <https://cloud.google.com/healthcare/docs/how-tos/dicomweb#deleting_a_study_series_or_instance Deleting a study, series, or instance>.
--
...
newHealthcareProjectsLocationsDatasetsDicomStoresStudiesSeriesDelete 
    ::  Core.Text
       -- ^  The path of the DeleteSeries request. For example, @studies\/{study_uid}\/series\/{series_uid}@. See 'dicomWebPath'.
    -> Core.Text
    ...
    -> HealthcareProjectsLocationsDatasetsDicomStoresStudiesSeriesDelete
newHealthcareProjectsLocationsDatasetsDicomStoresStudiesSeriesDelete dicomWebPath parent =
  HealthcareProjectsLocationsDatasetsDicomStoresStudiesSeriesDelete
    { xgafv = Core.Nothing
