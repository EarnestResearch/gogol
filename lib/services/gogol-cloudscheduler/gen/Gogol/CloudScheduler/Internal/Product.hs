...
newAppEngineHttpTarget 
    ::  AppEngineHttpTarget
newAppEngineHttpTarget
  = AppEngineHttpTarget{appEngineRouting = Core.Nothing,
                        body = Core.Nothing, headers = Core.Nothing,
                        httpMethod = Core.Nothing, relativeUri = Core.Nothing}
instance Core.FromJSON AppEngineHttpTarget where
        parseJSON
          = Core.withObject "AppEngineHttpTarget"
              (\ o ->
                 AppEngineHttpTarget Core.<$>
                   (o Core..:? "appEngineRouting") Core.<*> (o Core..:? "body")
                     Core.<*> (o Core..:? "headers")
...
newAppEngineRouting 
    ::  AppEngineRouting
newAppEngineRouting
  = AppEngineRouting{host = Core.Nothing, instance' = Core.Nothing,
          ...
          = Core.withObject "AppEngineRouting"
              (\ o ->
                 AppEngineRouting Core.<$>
                   (o Core..:? "host") Core.<*> (o Core..:? "instance") Core.<*>
                     (o Core..:? "service")
                     Core.<*> (o Core..:? "version"))
...
newHttpTarget 
    ::  HttpTarget
newHttpTarget
  = HttpTarget{body = Core.Nothing, headers = Core.Nothing,
               httpMethod = Core.Nothing, oauthToken = Core.Nothing,
               oidcToken = Core.Nothing, uri = Core.Nothing}
instance Core.FromJSON HttpTarget where
        parseJSON
          = Core.withObject "HttpTarget"
              (\ o ->
                 HttpTarget Core.<$>
                   (o Core..:? "body") Core.<*> (o Core..:? "headers") Core.<*>
                     (o Core..:? "httpMethod")
...
newJob 
    ::  Job
newJob
  = Job{appEngineHttpTarget = Core.Nothing,
        attemptDeadline = Core.Nothing, description = Core.Nothing,
        httpTarget = Core.Nothing, lastAttemptTime = Core.Nothing,
        name = Core.Nothing, pubsubTarget = Core.Nothing,
        retryConfig = Core.Nothing, satisfiesPzs = Core.Nothing,
        schedule = Core.Nothing, scheduleTime = Core.Nothing,
        state = Core.Nothing, status = Core.Nothing,
        timeZone = Core.Nothing, userUpdateTime = Core.Nothing}
...
instance Core.FromJSON OAuthToken where
        parseJSON
          = Core.withObject "OAuthToken"
              (\ o ->
                 OAuthToken Core.<$>
                   (o Core..:? "scope") Core.<*> (o Core..:? "serviceAccountEmail"))
...
newOidcToken 
    ::  OidcToken
newOidcToken
  = OidcToken{audience = Core.Nothing,
              serviceAccountEmail = Core.Nothing}
...
newPubsubMessage 
    ::  PubsubMessage
newPubsubMessage
  = PubsubMessage{attributes = Core.Nothing, data' = Core.Nothing,
                  messageId = Core.Nothing, orderingKey = Core.Nothing,
...
instance Core.FromJSON PubsubMessage where
        parseJSON
          = Core.withObject "PubsubMessage"
              (\ o ->
                 PubsubMessage Core.<$>
                   (o Core..:? "attributes") Core.<*> (o Core..:? "data") Core.<*>
                     (o Core..:? "messageId")
...
newPubsubTarget 
    ::  PubsubTarget
newPubsubTarget
  = PubsubTarget{attributes = Core.Nothing, data' = Core.Nothing,
...
-- | Settings that determine the retry behavior. For more information, see <https://cloud.google.com/scheduler/docs/configuring/retry-jobs Retry jobs>. By default, if a job does not complete successfully (meaning that an acknowledgement is not received from the handler, then it will be retried with exponential backoff according to the settings in RetryConfig.
--
...
data RetryConfig = RetryConfig
    {
      ...
      -- | The time between retries will double @max_doublings@ times. A job\'s retry interval starts at min/backoff/duration, then doubles @max_doublings@ times, then increases linearly, and finally retries at intervals of max/backoff/duration up to retry_count times. For examples, see <https://cloud.google.com/scheduler/docs/configuring/retry-jobs#max-doublings Retry jobs>. The default value of this field is 5.
    , maxDoublings :: (Core.Maybe Core.Int32)
      -- | The time limit for retrying a failed job, measured from the time when an execution was first attempted. If specified with retry/count, the job will be retried until both limits are reached. The default value for max/retry/duration is zero, which means retry duration is unlimited. However, if retry/count is also 0, a job attempt won\'t be retried if it fails.
    , maxRetryDuration :: (Core.Maybe Core.Duration)
      -- | The minimum amount of time to wait before retrying a job after it fails. The default value of this field is 5 seconds.
    , minBackoffDuration :: (Core.Maybe Core.Duration)
      -- | The number of attempts that the system will make to run a job using the exponential backoff procedure described by max/doublings. The default value of retry/count is zero. If retry/count is 0 (and if max/retry/duration is also 0), a job attempt won\'t be retried if it fails. Instead, Cloud Scheduler system will wait for the next scheduled execution time. Setting retry/count to 0 doesn\'t prevent failed jobs from running according to schedule after the failure. If retry/count is set to a non-zero number, Cloud Scheduler will retry the failed job, using exponential backoff, for retry/count times until the job succeeds or the number of retries is exhausted. Note that the next scheduled execution time might be skipped if the retries continue through that time. Values greater than 5 and negative values are not allowed.
...
newRetryConfig 
    ::  RetryConfig
newRetryConfig
  = RetryConfig{maxBackoffDuration = Core.Nothing,
                maxDoublings = Core.Nothing, maxRetryDuration = Core.Nothing,
                minBackoffDuration = Core.Nothing, retryCount = Core.Nothing}
