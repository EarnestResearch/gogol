         ...
         "userprofiles" Core.:>
           Core.Capture "profileId" Core.Int64 Core.:>
             "dynamicTargetingKeys" Core.:>
               Core.Capture "objectId" Core.Int64 Core.:>
                 Core.QueryParam "name" Core.Text Core.:>
                   Core.QueryParam "objectType" DynamicTargetingKeysDeleteObjectType
                     Core.:>
...
newDFAReportingDynamicTargetingKeysDelete 
    ...
    -> DFAReportingDynamicTargetingKeysDelete
newDFAReportingDynamicTargetingKeysDelete name objectId objectType
  profileId
  = DFAReportingDynamicTargetingKeysDelete{xgafv = Core.Nothing,
                                           accessToken = Core.Nothing, callback = Core.Nothing,
                                           name = name, objectId = objectId,
                                           objectType = objectType, profileId = profileId,
                                           uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DFAReportingDynamicTargetingKeysDelete
         where
        type Rs DFAReportingDynamicTargetingKeysDelete = ()
        type Scopes DFAReportingDynamicTargetingKeysDelete =
             '[Dfatrafficking'FullControl]
        requestClient DFAReportingDynamicTargetingKeysDelete{..}
          = go profileId objectId (Core.Just name) (Core.Just objectType)
              xgafv
              accessToken
              callback
              uploadType
