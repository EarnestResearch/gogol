                         ...
                         Core.QueryParam "uploadType" Core.Text Core.:>
                           Core.QueryParam "upload_protocol" Core.Text Core.:>
                             Core.QueryParam "alt" Core.AltJSON Core.:>
                               Core.ReqBody '[Core.JSON] CreativeAssetMetadata Core.:>
                                 Core.Post '[Core.JSON] CreativeAssetMetadata
       Core.:<|>
       "upload" Core.:>
         "dfareporting" Core.:>
           "v4" Core.:>
                               ...
                               Core.QueryParam "upload_protocol" Core.Text Core.:>
                                 Core.QueryParam "alt" Core.AltJSON Core.:>
                                   Core.QueryParam "uploadType" Core.Multipart Core.:>
                                     Core.MultipartRelated '[Core.JSON] CreativeAssetMetadata
...
newDFAReportingCreativeAssetsInsert advertiserId payload profileId
  = DFAReportingCreativeAssetsInsert{xgafv = Core.Nothing,
                                     accessToken = Core.Nothing, advertiserId = advertiserId,
...
instance Core.GoogleRequest DFAReportingCreativeAssetsInsert where
        ...
        type Scopes DFAReportingCreativeAssetsInsert =
             '[Dfatrafficking'FullControl]
        requestClient DFAReportingCreativeAssetsInsert{..}
          = go profileId advertiserId xgafv accessToken callback uploadType
              uploadProtocol
...
instance Core.GoogleRequest
         ...
         where
        type Rs (Core.MediaUpload DFAReportingCreativeAssetsInsert) =
             CreativeAssetMetadata
        type Scopes (Core.MediaUpload DFAReportingCreativeAssetsInsert) =
