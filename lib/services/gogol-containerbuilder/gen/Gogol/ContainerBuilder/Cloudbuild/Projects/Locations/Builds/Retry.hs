...
-- build will attempt to use the same object, which may or may not be available depending on the bucket\'s lifecycle management settings.
--
...
type CloudbuildProjectsLocationsBuildsRetryResource =
     "v1" Core.:>
       Core.CaptureMode "name" "retry" Core.Text Core.:>
...
instance Core.GoogleRequest
           CloudbuildProjectsLocationsBuildsRetry
         where
        type Rs CloudbuildProjectsLocationsBuildsRetry =
             Operation
        type Scopes CloudbuildProjectsLocationsBuildsRetry =
