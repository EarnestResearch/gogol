           ...
           Core.QueryParam "access_token" Core.Text Core.:>
             Core.QueryParam "allowMissing" Core.Bool Core.:>
               Core.QueryParam "callback" Core.Text Core.:>
                 Core.QueryParam "forceNewRevision" Core.Bool Core.:>
                   Core.QueryParam "updateMask" Core.FieldMask Core.:>
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "validateOnly" Core.Bool Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.ReqBody '[Core.JSON] GoogleCloudRunV2WorkerPool Core.:>
...
newRunProjectsLocationsWorkerPoolsPatch name payload
  = RunProjectsLocationsWorkerPoolsPatch{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, allowMissing = Core.Nothing,
                                         callback = Core.Nothing, forceNewRevision = Core.Nothing,
                                         name = name, payload = payload, updateMask = Core.Nothing,
                                         uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                         validateOnly = Core.Nothing}
instance Core.GoogleRequest RunProjectsLocationsWorkerPoolsPatch
         where
        type Rs RunProjectsLocationsWorkerPoolsPatch =
             GoogleLongrunningOperation
        type Scopes RunProjectsLocationsWorkerPoolsPatch =
             '[CloudPlatform'FullControl]
        requestClient RunProjectsLocationsWorkerPoolsPatch{..}
          = go name xgafv accessToken allowMissing callback forceNewRevision
              updateMask
              uploadType
              uploadProtocol
              validateOnly
